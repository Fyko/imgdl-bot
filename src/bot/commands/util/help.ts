...
export default class extends ImgDlCommand {
	...
	public async exec(
		msg: Message,
		{ arg }: { arg: undefined | ImgDlCommand | Category<string, ImgDlCommand> },
	): Promise<Message | Message[] | void> {
		const prefix = (await (this.handler.prefix as PrefixSupplier)(msg)) as string;
		...
		if (!arg) {
			...
			for (const category of this.handler.categories.values()) {
				...
				const commands = category
					...
					.map(
						cmd =>
							`\`${cmd.id !== 'help' && cmd.exec.toString().includes('NOT_IMPLEMENTED') ? '¹' : ''}${cmd.aliases[0]}\``,
					)
					.join(', ');
		...
		if (arg instanceof ImgDlCommand) {
			const embed = this.client.util
				.embed()
				.setColor(msg.guild?.me?.displayColor ?? this.client.config.color)
			...
			if (arg.description.flags)
				embed.addField(
					...
					arg.description.flags.map(
						({ description, flags }) => `${flags.map(f => codeb(f)).join(', ')}: ${description}`,
					),
				);
